        ...
        <>
            ...
            <nav className="bg-white px-6 md:px-12 lg:px-24 xl:px-40 py-4 flex items-center justify-between relative">
                ...
                <div className="hidden md:flex items-center bg-zinc-50 border border-zinc-200 rounded-full px-1 py-1 gap-2">
                    {navItems.map((item) => (
                        <a key={item} href="#" className={`px-4 py-1.5 rounded-full text-sm transition-colors ${item === 'Products' ? 'bg-white border border-zinc-200 font-medium text-zinc-800 hover:text-zinc-600' : 'text-zinc-500 hover:text-zinc-400' }`} >
                            {item}
                        </a>
                    ))}
                </div>
                ...
                {menuOpen && (
                    <div className="absolute top-full left-0 w-full bg-white border-t border-zinc-200 flex flex-col p-5 gap-1 md:hidden z-50">
                        {navItems.map((item) => (
                            <a key={item} href="#" className={`px-4 py-2.5 rounded-lg text-sm ${item === 'Products' ? 'bg-zinc-50 font-medium text-zinc-800' : 'text-zinc-500 hover:bg-zinc-50' }`} >
                                {item}
                            </a>
                        ))}
